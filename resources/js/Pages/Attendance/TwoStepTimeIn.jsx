...
export default function TwoStepTimeIn({ event }) {
    ...
    const handleVerifyStudentId = async (e) => {
        ...
        try {
            // Check if time-in window is active
            const now = new Date();
            const timeInStart = new Date(event.date + 'T' + event.time_in);
            const timeInEnd = new Date(timeInStart.getTime() + (event.time_in_duration || 30) * 60000);
            ...
            if (!timeInActive && timeOutActive) {
                // Time-in window closed but time-out is open, redirect to time-out
                toast("Time-in window closed. Redirecting to time-out...");
                setTimeout(() => {
                    router.visit(route("attendance-records.two-step-time-out", event.event_id));
                }, 2000);
                return;
            ...
            toast.success(`Welcome, ${data.member.firstname}!`);
    ...
    const handleFaceRecognition = async () => {
        ...
        try {
            ...
            // Record attendance
            await recordAttendance(result.faceId);
    ...
    // Record attendance
    const recordAttendance = async (faceId) => {
        if (isProcessing) return; // Prevent double submission
        ...
        try {
            ...
            if (data.success) {
                toast.success("Time In recorded successfully! Ready for next person.");
                ...
                // Show error message (e.g., already timed in)
    ...
    return (
        <SidebarProvider>
            ...
            <AppSidebar />
            <Head title="Time In - Two-Step Verification" />
                        ...
                        <Card>
                            ...
                            <CardContent>
                                ...
                                <div className="mb-6 p-4 bg-green-50 rounded-lg border border-green-200">
                                    ...
                                    <p className="text-green-700">Date: {event.date}</p>
                                    <p className="text-green-700">
                                        Time Window: {new Date(event.date + 'T' + event.time_in).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })} - {timeWindow.timeInEnd.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })}
                                    </p>
                                    {timeWindow.isActive ? (
                                        ...
                                        <div>
                                            ...
                                            <div className="flex gap-3">
                                                <button
                                                    ...
                                                    className="flex-1 px-6 py-3 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition disabled:bg-gray-400 disabled:cursor-not-allowed"
                                                >
                                                    {isProcessing ? "Scanning..." : "Scan Face & Record Attendance"}
                                                </button>
                                                <button
                                                    type="button"
                                                    onClick={handleBack}
